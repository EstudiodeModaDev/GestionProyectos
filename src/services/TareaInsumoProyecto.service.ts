...
export class TareaInsumoProyectoServicio {
  ...
  // ---------- mapping ----------
  private toModel(item: any): tareaInsumoProyecto {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f.Title, 
  ...
  // ---------- CRUD ----------
  async create(record: Omit<tareaInsumoProyecto, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<tareaInsumoProyecto   , 'ID'>>) {
    await this.ensureIds()
