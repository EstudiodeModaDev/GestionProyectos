...
export class ProyectosServices {
  ...
  // ---------- mapping ----------
  private toModel(item: any): ProjectSP {
    const f = item?.fields ?? {};
    return {
      Id: String(item?.id ?? ""),
      Title: f.Title,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<ProjectSP, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<ProjectSP, 'ID'>>) {
    await this.ensureIds()
  ...
  async getAll(opts?: GetAllOpts): Promise<ProjectSP[]> {
    await this.ensureIds()
