...
export class PlantillaTareaInsumoService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): plantillaTareaInsumo {
    const f = item?.fields ?? {};
    return {
      Id: String(item?.id ?? ""),
      Title: f.Title,
  ...
  // ---------- CRUD ----------
  async create(record: Omit<plantillaTareaInsumo, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<plantillaTareaInsumo, 'ID'>>) {
    await this.ensureIds()
  ...
  async getAll(opts?: GetAllOpts): Promise<plantillaTareaInsumo[]> {
    await this.ensureIds()
