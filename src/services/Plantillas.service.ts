...
export class AperturaTiendaService {
  ...
  constructor(
    ...
    listName = 'Tareas Apertura Tienda'     
  ) {
    this.graph = graph;
  ...
  // ---------- mapping ----------
  private toModel(item: any): apertura {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f.Title, 
  ...
  // ---------- CRUD ----------
  async create(record: Omit<apertura, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<apertura, 'ID'>>) {
    await this.ensureIds()
