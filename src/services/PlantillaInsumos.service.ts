...
export class PlantillaInsumoService {
  ...
  // ---------- mapping ----------
  private toModel(item: any): plantillaInsumos {
    const f = item?.fields ?? {};
    return {
        Id: String(item?.id ?? ''),
        Title: f.Title, 
  ...
  // ---------- CRUD ----------
  async create(record: Omit<plantillaInsumos, 'ID'>) {
  ...
  async update(id: string, changed: Partial<Omit<plantillaInsumos   , 'ID'>>) {
    await this.ensureIds()
